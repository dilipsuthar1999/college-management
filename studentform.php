	...
	<body>
		...
		<form  name="myForm" action="" method="POST">
			...
			<div class="input-group">
				...
				<br><input type="tel" name="phone" pattern="[0-9]{10}" required>
			</div>
			<div class="input-group">
				<br><label>10th Marks</label>
				<br><input type="tel" name="ten" pattern="[0-9]{2}" required>
			</div>
			<div class="input-group">
				<br><label>12th Marks</label>
				<br><input type="tel" name="tw" pattern="[0-9]{2}" required>
			</div>
			<div class="input-group">
				<br><label>Class</label>
				<br><input type="text" name="class" required>
			</div>
			<div class="input-group">
				<br><label>Course Name</label>
				<br><input type="text" name="cn" required>
			</div>
			<div class="input-group">
			...
			<div class="input-group">
				<br><button type="cancel" name="cancel" value="cancel"><a href="studentdetail.html" style="text-decoration:none; color:white;">Back</a></button>
			</div>
		</form>
	</body>
</html>
...
<?php
	...
	$tw=filter_input(INPUT_POST,'tw');
	$class=filter_input(INPUT_POST,'class');
	$cn=filter_input(INPUT_POST,'cn');
	$add=filter_input(INPUT_POST,'address');
	if(!empty($f) && !empty($m) && !empty($l) && !empty($dob) && !empty($g) && !empty($email) && !empty($pho) && !empty($ten) && !empty($tw) && !empty($class) && !empty($cn) && !empty($add))
	{
		$host="localhost";
		$dbusername="root";
		$dbpassword="";
		...
		{
			$queryl="SELECT email FROM student where email='{$email}'";
			...
			{
				$sql="INSERT INTO student(firstname,middlename,lastname,dob,gender,email,mobile,10th,12th,class,course,address)
					values ('$f','$m','$l','$dob','$g','$email','$pho','$ten','$tw','$class','$cn','$add')";
				if($conn->query($sql))
				{	
