	...
	<body>
		...
		<form  name="myForm" action="" method="POST">
			...
			<div class="input-group">
				...
				<br><input type="tel" pattern="{0-9}[10]" name="phone" required>
			</div>
			<div class="input-group">
			...
			<div class="input-group">
				...
				<br><input type="text" pattern="{0-9}" name="salary" required>
			</div>
			<div class="input-group">
			...
			<div class="input-group">
				<br><button type="cancel" name="cancel" value="cancel"><a href="staffdetail.html" style="text-decoration:none; color:white;">Cancel</a></button>
			</div>
		</form>
	</body>
</html>
...
<?php
	...
	if(!empty($f) && !empty($m) && !empty($l) && !empty($dob) && !empty($g) && !empty($email) && !empty($pho) && !empty($q) && !empty($add) && !empty($sal))
	{
		...
		{
			$queryl="SELECT email FROM staff where email='{$email}'";
			...
			{
				$sql="INSERT INTO staff(firstname,middlename,lastname,dob,gender,email,mobile,q,address,salary)
