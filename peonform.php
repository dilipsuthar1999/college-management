	...
	<body>
		...
		<form  name="myForm" action="" method="POST">
			...
			<div class="input-group">
				...
				<br><input type="text" name="dob" required>
			</div>
			<div class="input-group">
...
<?php
	...
	if(!empty($f) && !empty($m) && !empty($l) && !empty($dob) && !empty($g) && !empty($email) && !empty($pho) && !empty($q) && !empty($add) && !empty($sal))
	{
		...
		{
			$queryl="SELECT email FROM peon where email='{$email}'";
			...
			{
				$sql="INSERT INTO peon(firstname,middlename,lastname,dob,gender,email,mobile,q,address,salary)
